...
int main() {
    ...
    while(cursor <= cnt || size(&ready_queue) || size(&execute_queue)) { // 프로세스가 남아있다면.
        ...
        if(size(&execute_queue)) { // 실행을 위한 프로세스 존재
            ...
            if(check_isend(&execute_queue)) { // burst_time이 남아있지 않을 경우
            ...
            else if(slice == time_quantum) { // 타임 퀀텀만큼 실행
                slice = 0;
                if(!size(&ready_queue)) continue;
                process remove_proc = pop(&execute_queue); // execute_queue에서 제거  
                ...
                while(cursor <= cnt && proc[cursor - 1].arrive_time <= cur_time + 1) { // 현재 시간 >= 도착시간
                    push(&ready_queue, proc[cursor - 1]); // 레디 큐에 푸쉬
                    waiting[proc[cursor - 1].pnum][0] = cur_time + 1; // cursor 프로세스는 waiting을 시작하게 됨.
                    getcolor(cursor);
                    printf("( Process [%d] is inserted in Ready Queue )\n", cursor);
                    cursor++;
                }

                push(&ready_queue, remove_proc); // 레디 큐에 푸쉬
                waiting[remove_proc.pnum][0] = cur_time + 1; // cursor 프로세스는 waiting을 시작하게 됨.
                getcolor(remove_proc.pnum);
                printf("( Process [%d] is inserted in Ready Queue )\n", remove_proc.pnum);
            }
        }
