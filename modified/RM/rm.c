...
int main() {
    ...
    Heap ready_queue, execute_queue;
    ...
    while(cursor <= cnt || size(&ready_queue) || size(&execute_queue)) { // 프로세스가 남아있다면.
        ...
        if(size(&ready_queue) > 0 && size(&execute_queue) > 0) {
            if(print_top_rate(&ready_queue) < print_top_rate(&execute_queue)) {
                process new_execute = pop(&ready_queue);
                process new_ready = pop(&execute_queue);
                waiting[new_execute.pnum][1] = cur_time;
                waiting_time[new_execute.pnum] += waiting[new_execute.pnum][1] - waiting[new_execute.pnum][0];
                // 새로 실행 될 프로세스의 waiting time 처리
                printf(ANSI_COLOR_RESET "---------Context Switching Occured---------\n");
                context_switching++;
                getcolor(new_execute.pnum);
                printf("( Process [%d] is starting at %d )\n",new_execute.pnum, cur_time);
                executing[new_execute.pnum][0] = cur_time; // new_execute 실행 시작  
                push(&execute_queue, new_execute);
                fprintf(fd, "%s %d %d %s\n",new_execute.pname, waiting[new_execute.pnum][0], cur_time, "Waiting");

                getcolor(new_ready.pnum);
                executing[new_ready.pnum][1] = cur_time;
                waiting[new_ready.pnum][0] = cur_time;
                printf("( Process [%d] is paused at %d )\n",new_ready.pnum, cur_time);
                push(&ready_queue, new_ready);
                fprintf(fd, "%s %d %d %s\n", new_ready.pname, executing[new_ready.pnum][0], executing[new_ready.pnum][1], "Executing");
            }
        }
    ...
    for(int i = 1 ; i <= cnt ; i++) {
        ...
        if(isdead[i]) {
            printf(ANSI_COLOR_RED);
            printf("This process did not meet the deadline.\n");
            printf(ANSI_COLOR_RESET);
        }
        makeline2();
    }
    ...
    (double)tot_wait/cnt, (double)tot_response/cnt, (double)tot_turnaround/cnt, context_switching);
    return 0;
}